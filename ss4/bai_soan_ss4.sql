...
select*from jame;
select
c.id_classes,c.name_classes, count(s.classes_id) as 'quantity'
from classes c
left join student s
...
select c.name_classes, max(s.point_student)
from classes c
left join student s
on c.id_classes=s.classes_id
group by id_classes;
-- Tình điểm trung bình  của từng lớp ( yêu cầu viết code ra giấy )
select c.name_classes, avg(s.point_student)
from classes c
left join student s
on c.id_classes=s.classes_id
group by id_classes;
